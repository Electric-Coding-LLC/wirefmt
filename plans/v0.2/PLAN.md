# Plan

This `v0.2` release will be completed by working through these steps in order.

## Scope

| Step | Started | Completed |
| --- | --- | --- |
| [01-define-release-scope](01-scope/01-define-release-scope.md) | 2026-04-17 | 2026-04-17 |

## Product And Contracts

| Step | Started | Completed |
| --- | --- | --- |
| [01-define-agent-tool-surface](02-product-and-contracts/01-define-agent-tool-surface.md) | 2026-04-17 | 2026-04-17 |
| [02-define-result-contracts](02-product-and-contracts/02-define-result-contracts.md) | 2026-04-17 | 2026-04-17 |

## Implementation

| Step | Started | Completed |
| --- | --- | --- |
| [01-add-mcp-lint-tool](03-implementation/01-add-mcp-lint-tool.md) | 2026-04-17 | 2026-04-17 |
| [02-align-cli-and-mcp-output](03-implementation/02-align-cli-and-mcp-output.md) | 2026-04-17 | 2026-04-17 |
| [03-polish-agent-setup-docs](03-implementation/03-polish-agent-setup-docs.md) | 2026-04-17 | 2026-04-17 |

## Testing

| Step | Started | Completed |
| --- | --- | --- |
| [01-add-agent-fixture-coverage](04-testing/01-add-agent-fixture-coverage.md) | 2026-04-17 | 2026-04-17 |
| [02-verify-cli-and-mcp-parity](04-testing/02-verify-cli-and-mcp-parity.md) | 2026-04-17 | 2026-04-17 |

## Release

| Step | Started | Completed |
| --- | --- | --- |
| [01-prepare-v0.2-release](05-release/01-prepare-v0.2-release.md) | 2026-04-17 | 2026-04-17 |

## Notes

- `v0-poc` is complete and treated as the baseline.
- `v0.2` is a focused product release, not a second proof-of-concept.
- Theme: make `wirefmt` feel complete for agent use through CLI and MCP parity.
