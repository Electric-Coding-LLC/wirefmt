# Define adjacent-box contracts

[Back to Execution Map](./EXECMAP.md)

## Goal

Define how adjacent sibling boxes are detected and how `format` and `lint`
should behave for the supported shape and nearby unsupported variants.

## Contract Decision

Treat the supported `v0.4` shape as two independently normalizable boxes that
happen to share one block:

- The block must contain exactly two boxes on every nonblank row.
- The boxes must appear in the same order on every row, separated by exactly
  one space column.
...
- `format` should split the block into left-box and right-box sub-blocks,
  ...
  rows with exactly one space column.
...
- `v0.4` should not add new CLI flags, MCP tools, or multi-box-specific issue
  codes.
...
- Specify the structural rules that distinguish the supported adjacent-box
  shape from unsupported multi-box inputs.
- Define the expected formatter result for the supported shape, including
  spacing, width handling, and block preservation rules.
...
- Do not add new commands, flags, or MCP tools for `v0.4`.
- Keep existing warning and issue behavior stable unless a contract change is
  necessary and documented by this step.

## Exit Criteria

- The supported adjacent-box contract is precise enough to implement without
  reopening scope.
